...
export const ResetPasswordForm = () => {
  ...
  const onSubmit = async (data: ResetPasswordFormData) => {
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        ...
        setTimeout(() => router.push("/admin/forgot-password"), 1500);
        return;
      }
  ...
  return (
    <form className="grid grid-cols-1 gap-y-[15px]" onSubmit={handleSubmit(onSubmit, (errs) => {
      ...
      <div>
        <label htmlFor="password" className="font-[500] text-[14px] text-black mb-[5px] block">New Password *</label>
        <div className="relative">
          <input type={showPassword ? "text" : "password"} id="password" placeholder="Enter new password" autoComplete="new-password"
      ...
      <div>
        <label htmlFor="confirmPassword" className="font-[500] text-[14px] text-black mb-[5px] block">Confirm Password *</label>
        <div className="relative">
          <input type={showConfirm ? "text" : "password"} id="confirmPassword" placeholder="Confirm new password" autoComplete="new-password"
            className="w-full h-[46px] rounded-[8px] border border-[#DEDEDE] px-[20px] pr-[50px] font-[500] text-[14px] text-black focus:border-[#0088FF] focus:ring-2 focus:ring-[#0088FF]/20 transition-all duration-200"
            {...register("confirmPassword")} />
          <button type="button" onClick={() => setShowConfirm(!showConfirm)}
            className="absolute right-[15px] top-1/2 -translate-y-1/2 text-[#666] hover:text-[#333] cursor-pointer">
            {showConfirm ? <FaEyeSlash className="text-[18px]" /> : <FaEye className="text-[18px]" />}
          </button>
        </div>
