...
export const OtpPasswordForm = () => {
  ...
  useEffect(() => {
    ...
    submitTimerRef.current = window.setTimeout(() => { handleSubmit(onSubmit)(); }, 300);
    return () => { if (submitTimerRef.current) window.clearTimeout(submitTimerRef.current); };
  ...
  const onSubmit = async (data: OtpPasswordFormData) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/auth/otp-password`, {
        method: "POST",
  ...
  if (!isReady) return <div className="text-center py-[20px]"><p className="text-[#666]">Loading...</p></div>;

  const otpFieldProps = register("otp");

  return (
    <form className="grid grid-cols-1 gap-y-[14px]" onSubmit={handleSubmit(onSubmit, (errs) => {
      ...
      <div>
        ...
        <p className="text-[12px] text-[#777] mt-[6px] text-center">6-digit code. Check your spam folder too.</p>
      </div>
      <button type="submit" disabled={isSubmitting}
        className="w-full h-[48px] rounded-[10px] bg-gradient-to-r from-[#0088FF] to-[#0066CC] font-[700] text-[16px] text-white hover:from-[#0077EE] hover:to-[#0055BB] cursor-pointer transition-all duration-200 active:scale-[0.98]">
        Verify OTP
      </button>
      <div className="text-center text-[13px]">
        Didn&apos;t get it?{" "}
        <Link href="/admin/forgot-password" className="font-[600] text-[#0088FF] hover:underline">Resend OTP</Link>
      </div>
    </form>
  );
};
