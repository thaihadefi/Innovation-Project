...
export const ResetPasswordForm = () => {
  ...
  const onSubmit = async (data: ResetPasswordFormData) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/candidate/reset-password`, {
        method: "POST",
      ...
      if (res.status === 401 || res.status === 403) {
        ...
        setTimeout(() => window.location.href = "/candidate/forgot-password", 1500);
        return;
      }
      if (result.code == "error") toast.error(result.message);
      if (result.code == "success") {
        toast.success(result.message);
        setTimeout(() => window.location.href = "/candidate/login", 500);
      }
    } catch {
