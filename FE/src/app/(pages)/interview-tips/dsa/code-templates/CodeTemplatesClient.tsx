...
export function CodeTemplatesClient({ initialTab }: { initialTab?: CodeTab }) {
  const [activeTab, setActiveTab] = useState<CodeTab>(
    initialTab && codeTabs.includes(initialTab) ? initialTab : "C++"
  );

  const handleTabChange = (tab: CodeTab) => {
    setActiveTab(tab);
    if (typeof document !== "undefined") {
      document.cookie = `interview_tips_code_tab=${encodeURIComponent(tab)}; Path=/; Max-Age=2592000; SameSite=Lax`;
    }
  };
