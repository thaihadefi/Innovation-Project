      ...
      {isVerified && (
        <>
          ...
          {loading ? (
            ...
            <ErrorRetryState message={errorMessage} onRetry={() => fetchCVList(currentPage, activeKeyword)} />
          ) : (pagination?.totalRecord || 0) === 0 ? (
            <EmptyCardState
              icon={<FaBriefcase className="text-[30px]" />}
              title="No applications found"
              description={activeKeyword ? "Try adjusting your search filters." : "You haven't submitted any applications yet."}
              actions={
                activeKeyword ? (
                  <button
                    onClick={() => {
