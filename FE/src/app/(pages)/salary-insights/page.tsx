...
export default function SalaryInsightsPage() {
  ...
  const [byPosition, setByPosition] = useState<SalaryInsight[]>([]);
  const [byTechnology, setByTechnology] = useState<SalaryInsight[]>([]);
  const [byCity, setByCity] = useState<SalaryInsight[]>([]);

  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/salary/insights`)
      .then(res => res.json())
      .then(data => {
        if (data.code === "success") {
          setOverview(data.overall);
          setByPosition(data.byPosition);
