        ...
        <table className="w-full text-[14px]">
          <thead>
            <tr className="border-b border-[#F0F0F0] text-[#666]">
              ...
              <th className="text-left px-[16px] py-[12px] font-[600]">Verified</th>
          ...
          <tbody>
            {initialCandidates.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-[40px] text-[#999]">No candidates found.</td></tr>
            ) : initialCandidates.map((c) => (
              <tr key={c._id} className="border-b border-[#F9F9F9] hover:bg-[#FAFAFA]">
                <td className="px-[16px] py-[12px] font-[500]">{c.fullName}</td>
                ...
                <td className="px-[16px] py-[12px]">
                  <div className="flex gap-[8px]">
                    <button disabled={!!loading}
                      ...
                      className="text-[12px] px-[10px] py-[4px] rounded-[6px] border border-[#0088FF] text-[#0088FF] hover:bg-[#0088FF] hover:text-white transition-all cursor-pointer disabled:opacity-50">
                      {c.isVerified ? "Unverify" : "Verify"}
                    </button>
                    <button disabled={!!loading}
                      onClick={() => patchAction(c._id, "status", { status: c.status === "active" ? "inactive" : "active" })}
                      className={`text-[12px] px-[10px] py-[4px] rounded-[6px] border transition-all cursor-pointer disabled:opacity-50 ${
                        c.status === "active"
