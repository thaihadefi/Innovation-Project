...
// Candidate-only handlers rely on the verifyTokenCandidate route guard; only
// getCompanyReviews (public) and reportReview (verifyTokenAny) are exceptions.
...
export const getCompanyReviews = async (req: RequestAccount<{
    companyId: string;
}>, res: Response) => {
...
export const updateReview = async (req: RequestAccount, res: Response) => {
    ...
    const result = await reviewService.updateReviewService(reviewId, candidate._id, candidate.fullName || "A candidate", candidate.isVerified, body);
    res.status(result.status).json(result);
};
...
export const reportReview = async (req: RequestAccount, res: Response) => {
    ...
    const result = await reviewService.reportReviewService(reviewId, reason, reporterId, reporterType, isGuest ? clientIp : null);
    res.status(result.status).json(result);
};
