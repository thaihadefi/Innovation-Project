...
import { setNoCacheHeaders } from "../../helpers/response.helper";
...
export const loginPost = async (req: Request, res: Response) => {
    ...
    const result = await adminAuthService.loginAdminService(String(body.email || ""), String(body.password || ""), body.rememberPassword);
    if (result.token) {
...
export const otpPasswordPost = async (req: Request, res: Response) => {
    ...
    const result = await adminAuthService.verifyOtpAdminService(String(body.email || ""), String(body.otp || ""));
    if (result.token) {
...
export const resetPasswordPost = async (req: RequestAdmin, res: Response) => {
    ...
    const result = await adminAuthService.resetPasswordAdminService(admin._id.toString(), String(body.password || ""));
...
export const checkAuth = async (req: RequestAdmin, res: Response) => {
    ...
    const admin = req.admin!;
    const result = await adminAuthService.checkAdminAuthService(admin, req.permissions ?? undefined);
    res.json(result);
};
