  ...
  try {
    ...
    if (!isOwnerViewing) {
      ...
      try {
        ...
        void Job.updateOne({ _id: jobInfo._id }, { $inc: { viewCount: 1 } }).exec().catch((err) => console.warn('[Job] Failed to increment viewCount:', err));
      } catch (error: any) {
        if (error?.code === 11000) {
          // Duplicate view (same user already viewed today) - silently ignore
        } else {
      ...
      if (job) {
        ...
        if (company?.email) {
          ...
          const emailContent = `
            ...
            <p>View the application: <a href="${(process.env.FRONTEND_URL || 'http://localhost:3069').replace(/\/$/, '')}/company-manage/cv/detail/${newRecord.id}">Click here</a></p>
          `;
