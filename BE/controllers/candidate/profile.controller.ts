...
export const profilePatch = async (req: RequestAccount, res: Response) => {
    ...
    const result = await candidateProfileService.updateCandidateProfileService(candidate, body, req.file ? { path: req.file.path } : undefined);
...
export const requestEmailChange = async (req: RequestAccount, res: Response) => {
    ...
    const result = await candidateProfileService.requestCandidateEmailChangeService(candidate, String(body.newEmail || ""));
    res.status(result.status).json({ code: result.code, message: result.message });
};
export const verifyEmailChange = async (req: RequestAccount, res: Response) => {
    ...
    const result = await candidateProfileService.verifyCandidateEmailChangeService(candidate._id, String(body.otp || ""));
